...
def assessment_is_finished(submission_uuid, requirements):
    """
    When the submitter has finished assessing the training
    examples, then this step is finished.

    Args:
        Same as `submitter_is_finished()`
...
def validate_training_examples(rubric, examples):
    ...
    Returns:
        tuple of `(is_valid, errors)` where `is_valid` is a boolean indicating
        whether the examples are valid, and `errors` is a list of unicode strings
        describing validation errors.
    ...
    Example usage:
        ...
        >>> ]
        >>>
        >>> is_valid, errors = validate_training_examples(rubric, examples)
    ...
    try:
        ...
        msg = u"Could not parse serialized rubric"
        return False, [msg]
    ...
    for order_num, example_dict in enumerate(examples, start=1):
        ...
        if not is_format_valid:
            format_errors = [
                "Example {} has a validation error: {}".format(order_num, error)
            ...
            for criterion_name, option_name in options_selected.iteritems():
                if criterion_name in criteria_options:
                    ...
                    msg = u"Example {} has an extra option for \"{}\"".format(
            ...
            for missing_criterion in set(criteria_options.keys()) - set(options_selected.keys()):
                msg = u"Example {} is missing an option for \"{}\"".format(
